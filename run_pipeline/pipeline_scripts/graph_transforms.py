...
class GraphTransformDirectory:
    def __init__(self, transform_directory, json_path, output_directory, input_rotation_unit, 
                 plot_tile = "Motion Characterization Plots", threshold_as_percent = None,
                 threshold_as_mm = None, transform_suffix = ".tfm", 
                 framewise_displacements = False, do_not_plot = False):
        
        os.makedirs(output_directory, exist_ok=True)

        if threshold_as_mm != None and threshold_as_percent != None:
            print(f"\nERROR: Please enter a value for either: 'threshold_as_mm' or 'threshold_as_percent' not both")
            exit(0)
        elif threshold_as_percent == None and threshold_as_mm == None:
            threshold_as_percent = 25
        
        self.threshold_as_mm = threshold_as_mm
        ...
        if threshold_as_percent:
            ...
            self.threshold_as_mm = self.get_threshold_in_mm(json_path, threshold_as_percent)
            
            self.motion_flagged_volumes = self.get_motion_flagged_volumes(
                num_volumes=num_volumes,
                num_slice_groups=self.num_slice_groups,
                displacement_values=self.displacements,
                threshold_as_mm=self.threshold_as_mm
            )
            print(f"Motion Flagged Volumes:\n{self.motion_flagged_volumes}")

            self.write_flags_to_text_file(
                self.motion_flagged_volumes,
                output_text_file_path=os.path.join(output_directory, f"motion_flags_{threshold_as_percent}-percent-of-voxelsize-threshold.txt")
            )        
        ...
        if threshold_as_percent:
            output_filename = f"{threshold_as_percent}-percent-threshold_plot.png"
        if self.threshold_as_mm:
            output_filename = f"{self.threshold_as_mm}mm-threshold_plot.png"
        output_plot_path = os.path.join(output_directory, output_filename)
    ...
    def get_threshold_in_mm(self, json_path, threshold_as_percent):
        with open(json_path, mode='r') as f:
            threshold_as_mm = json.load(f)['SpacingBetweenSlices'] * (threshold_as_percent / 100)
            print(f"Threshold Displacement ({threshold_as_percent}% of Voxel Width): {round(threshold_as_mm, 4)} mm")
            return threshold_as_mm
 

    def plot_displacements(self, displacement_axes, displacements, xaxis_ticks, xaxis_tick_labels, threshold_as_percent, threshold_as_mm, motion_flagged_volumes, num_slice_groups):
        ...
        if threshold_as_mm:
            ...
            displacement_axes.axhline(y=threshold_as_mm, color='red', linestyle='--', label=f"Motion Threshold:\n{threshold_as_mm} mm / {threshold_as_percent} Percent of Voxel Width")
    ...
    def get_motion_flagged_volumes(self, num_volumes, num_slice_groups, displacement_values, threshold_as_mm):
        ...
        for volume_num in range(num_volumes):
            ...
            if any([displacement > threshold_as_mm for displacement in displacements_at_this_volume]):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Threshold as a percent of the width of a single voxel. Default is 25 Percent. " + \
             "Enter a value for 'threshold_as_percent' or 'threshold_as_mm' or neither."
    )
    parser.add_argument(
        "--threshold_as_mm",
        required=False,
        type=float,
        default=None,
        help="Threshold in mm. Enter a value for 'threshold_as_percent' or 'threshold_as_mm' or neither."
    )
    parser.add_argument(
    ...
    GraphTransformDirectory(
        ...
        threshold_as_mm=args.threshold_as_mm,
