...
class StartMotionCorrection:
    def __init__(self, 
        ...
        scrubbing_threshold_in_mm = self.get_scrub_threshold_in_mm(input_nifti_image_path, scrubbing_threshold)
    ...
    def get_scrub_threshold_in_mm(self, input_nifti_image_path, scrubbing_threshold):
        """
        ...
        """
        d_x, d_y, d_z, d_t = sitk.ReadImage(input_nifti_image_path).GetSpacing()
        
        diameter = math.sqrt(d_x ** 2 + d_y ** 2 + d_x ** 2)
        print(f"Voxel Diameter: {diameter}mm")

        return diameter * (scrubbing_threshold / 100)
