import os
import statistics 
import numpy as np
import SimpleITK as sitk
from typing import Iterable
from scipy.optimize import minimize

class ParameterSearch:
    
    def __init__(self, 
                 reference_volume: str | sitk.Image,  # pyright: ignore[reportRedeclaration]
                 slices: list[str] | list[sitk.Image],  # pyright: ignore[reportRedeclaration]
                 initial_versor_parameters: Iterable[float], 
                 fixed_parameters: Iterable[float],
                 output_transform_filename: str,
                 output_directory: str,
                 max_iterations: int = 200,
                 verbose: bool = False):

        self.verbose: bool = verbose
        os.makedirs(output_directory, exist_ok=True)

        """
        =================================================================
        Load data as SimpleITK Images (if not already loaded)
        =================================================================
        """
        if isinstance(reference_volume, str):
            reference_volume: sitk.Image = sitk.ReadImage(reference_volume)
        elif not isinstance(reference_volume, sitk.Image):
            raise TypeError("arg: 'reference_volume' must be a SimpleITK Image or an Existing NiFTI File Path")

        if all(isinstance(slice, str) for slice in slices):
            slices: list[sitk.Image] = [sitk.ReadImage(slice_path) for slice_path in sorted(slices)]
        elif not all(isinstance(slice, sitk.Image) for slice in slices):
            raise TypeError("arg: 'slices' must be a list of SimpleITK Images or a list of Existing NiFTI File Paths")
        
        
        """
        =================================================================
         Run optimization, minimize the cost function 
         (the negative mutual information value)
        =================================================================
        """
        result = minimize(
            fun=self.objective,
            x0=initial_versor_parameters,
            args=(fixed_parameters, reference_volume, slices),
            method='Powell',
            options={'maxiter': max_iterations, 'disp': False}   
        )
        if self.verbose:
            print(result.message + "   " * 20)
            print(f"Function evaluations: {result.nfev}")
            print(f"Versor Parameters with the Highest Mutual Info:\n{', '.join(str(val) for val in result.x)}")
            print(f"Mutual Information of the Parameters: { -result.fun}")
        
        """
        =================================================================
        Create the output transform
        =================================================================
        """
        self.found_parameters: list[float] = list(result.x)
        self.output_transform_path: str = os.path.join(output_directory, output_transform_filename)
        sitk.WriteTransform(
            transform=self.create_transform(
                versor_parameters=self.found_parameters,
                fixed_parameters=fixed_parameters),
            filename=self.output_transform_path
        )


    def objective(self, 
                  versor_parameters: Iterable[float], 
                  fixed_parameters: Iterable[float], 
                  reference_volume: sitk.Image, 
                  slices: list[sitk.Image]) -> float:

        """
        =================================================================
        1. Create the transform with the parameters we will test
        =================================================================
        """
        transform: sitk.VersorRigid3DTransform = self.create_transform(
            versor_parameters=versor_parameters,
            fixed_parameters=fixed_parameters
        )

        mutual_information_values_at_slice_group: list[float] = []
        for slice in slices:
            """
            =================================================================
            2. Apply the transform to the slices
            =================================================================
            """
            resampled_ref_to_moving_slice: sitk.Image = self.apply_transform(
                transform=transform,
                reference_volume=reference_volume,
                slice=slice
            )


            """
            =================================================================
            3. Evaluate the parameters by getting the mutual information 
               between the resampled slice and the target slice
            =================================================================
            """
            mutual_information_values_at_slice_group.append(
                self.get_mutual_information(
                    ref_data=sitk.GetArrayFromImage(resampled_ref_to_moving_slice),
                    target_data=sitk.GetArrayFromImage(slice)
                )
            )

        """
        ==================================================================
        4. Mean across slices in the slice group
        ==================================================================
        """
        mutual_information: float = statistics.mean(mutual_information_values_at_slice_group) 

        if self.verbose:
            print(
                f"Mutual Information: {round(mutual_information, 4)}, Parameters: {', '.join([str(round(parameter, 4)) for parameter in list(versor_parameters)])}                                 ",
                end='\r'
            )

        """
        ==================================================================
        5. Return negative of mutual information 
           (our optimizer is looking for the minimum of the cost function)
        ==================================================================
        """
        return -mutual_information


    def create_transform(self, 
                         versor_parameters: Iterable[float], 
                         fixed_parameters: Iterable[float]
                         ) -> sitk.VersorRigid3DTransform:
        
        axis: list[float] = [versor_parameters[0], versor_parameters[1], versor_parameters[2]] # pyright: ignore[reportRedeclaration, reportAssignmentType, reportIndexIssue]
        angle: float = np.linalg.norm(axis) # pyright: ignore[reportAssignmentType]

        transform: sitk.VersorRigid3DTransform = sitk.VersorRigid3DTransform()
        # No rotation
        if angle == 0:
            transform.SetRotation([1, 0, 0], 0.0)  # arbitrary axis, zero angle
        else:
            axis: float = [x / angle for x in axis] # pyright: ignore[reportGeneralTypeIssues, reportAssignmentType]
            transform.SetRotation(axis, angle)

        transform.SetTranslation([versor_parameters[3], versor_parameters[4], versor_parameters[5]]) # pyright: ignore[reportIndexIssue]
        transform.SetCenter(fixed_parameters) 

        return transform


    def apply_transform(self, 
                        transform: sitk.VersorRigid3DTransform, 
                        reference_volume: sitk.Image, 
                        slice: sitk.Image) -> sitk.Image:

        output_pixel_type = slice.GetPixelID()
        resampleFilter = sitk.ResampleImageFilter()
        resampleFilter.SetInterpolator(sitk.sitkLinear)
        resampleFilter.SetTransform(transform)
        resampleFilter.SetOutputPixelType(output_pixel_type)
        resampleFilter.SetDefaultPixelValue(0.0)
        resampleFilter.SetReferenceImage(slice)
        
        return resampleFilter.Execute(reference_volume)


    def get_mutual_information(self, 
                               ref_data: np.ndarray, 
                               target_data: np.ndarray, 
                               nbins: int = 64
                               ) -> float:
        """
        Function based off of: 
        https://matthew-brett.github.io/teaching/mutual_information.html
        """

        hist_2d, _, _ = np.histogram2d(
            ref_data.ravel(),
            target_data.ravel(),
            bins=nbins)

        # Convert bin counts in the joint hisogram to probability values
        # by dividing each bin count by the total number of samples 
        prob_xy: np.ndarray = hist_2d / float(np.sum(hist_2d))

        # A marginal distribution is the distribution of one variable ignoring the other
        # Compute the marginal for x over y
        prob_x: np.ndarray = np.sum(prob_xy, axis=1)
        # Compute the marginal for y over x
        prob_y: np.ndarray = np.sum(prob_xy, axis=0)

        # Compute the product of marginals
        # This is what the joint distribution would be if X and Y were independent.
        px_py: np.ndarray = prob_x[:, None] * prob_y[None, :]

        nzs: np.ndarray = prob_xy > 0 # Only non-zero pxy values contribute to the sum

        # prob_xy[nzs] -> gets the nonzero joint probabilities 
        # px_py[nzs] -> gets the matching independent joint probabilities
        return np.sum(prob_xy[nzs] * np.log(prob_xy[nzs] / px_py[nzs]))


    def get_output_parameters(self) -> list[float]:
        return self.found_parameters
    

    def get_output_transform_path(self) -> str:
        return self.output_transform_path
        

if __name__ == '__main__':
    import argparse

    default_output_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs")

    parser = argparse.ArgumentParser(
        description="Motion Characterization of a Slice Group from a Reference Volume via Maximization of Mutual Information"
    )
    parser.add_argument(
        "--reference_volume_path", 
        required=True
    )
    parser.add_argument(
        "--slice_group_paths", 
        required=True,
          nargs='+'
    )
    parser.add_argument(
        "--initial_versor_parameters",
        required=True,
        nargs='+',
        type=float,
        help="VERSOR PARAMETERS ONLY"
    )
    parser.add_argument(
        "--fixed_parameters",
        required=True,
        nargs='+',
        type=float
    )
    parser.add_argument(
        "--output_transform_filename", 
        required=True
    )
    parser.add_argument(
        "--output_directory", 
        required=False,
        default=default_output_directory,
        help=f"Default: {default_output_directory}"
    )
    args = parser.parse_args()

    ParameterSearch(
        reference_volume=os.path.abspath(args.reference_volume_path),
        slices=[
            os.path.abspath(slice_path)
            for slice_path in args.slice_group_paths
        ],
        initial_versor_parameters=args.initial_versor_parameters,
        fixed_parameters=args.fixed_parameters,
        output_directory=os.path.abspath(args.output_directory),
        output_transform_filename=args.output_transform_filename
    )